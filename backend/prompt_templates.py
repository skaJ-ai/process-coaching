...
- "~여부", "~인가?", "~인지?", "~있는가?", "~했는가?", "~필요한가?", "~가능한가?" 형식은 모두 유효 — 추가 MODIFY 제안 금지.
"""
...
4. **Decision 노드 중복 방지**: "~여부"와 "~인가?/~인지?/~있는가?/~했는가?"는 같은 판단의 다른 표현입니다.
   이미 같은 주제의 Decision 노드가 존재하면, 표현 형식이 달라도 ADD 또는 MODIFY 하지 마세요.
   예: "보안 서약이 필요한가?"가 있으면 → "보안 서약 여부"를 ADD하거나 MODIFY 제안 금지.
"""
...
3. [MODIFY] 판단 기준 누락: 분기 노드 라벨이 "~한다" 형식이거나 아무 조건도 없는 경우만 해당.
   ⛔ 면제: "~여부", "~인가?", "~인지?", "~있는가?", "~했는가?", "~필요한가?", "~가능한가?" 형식이면 이미 유효한 판단 기준 — MODIFY 제안 금지.
