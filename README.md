...
실무 플로우(35+ 노드) 시뮬레이션에서 도출된 기능 개선 항목. 현재 도구의 코칭 범위가 **라벨 품질·구조 완결성**에 한정되어 있고, 실무에서 진짜 어려운 **레이아웃·배치·복잡도 관리**를 도울 수 없는 문제를 해결하기 위한 백로그.
...
| 현상 | 하나의 캔버스에 3개 서브플로우(취소·차수변경·인프라관리)가 혼재 시, 어떤 노드가 어떤 플로우에 속하는지 시각적으로 불명확 |
| 제안 | 반투명 배경 영역(Group Node)으로 관련 노드를 시각적으로 묶고 제목 표시 |
| 구현 방향 | ReactFlow의 `type: 'group'` 노드 또는 커스텀 GroupNode 컴포넌트. 노드의 `parentNode` 속성으로 소속 관리 |
...
| 구현 방향 | `FlowNodeData`에 `childFlowId` 필드 추가, store에 `flows: Record<string, {nodes, edges}>` 멀티 캔버스 구조. 브레드크럼 네비게이션으로 상위↔하위 이동 |
| 비고 | 데이터 구조 변경이 크므로 신중하게 설계 필요. S-11(50노드 초과 시 분해 권장) 규칙과 연동하면 자연스러운 UX |
...
| 현상 | AI 코치가 라벨·구조는 코칭하지만, "이 노드 배치가 읽기 어렵다"는 피드백을 줄 수 없음 |
| 제안 | 엣지 교차 수, 역방향 엣지 비율, 노드 밀집도를 계산하여 레이아웃 품질 점수를 대시보드에 표시. 임계값 초과 시 "자동 정렬을 사용해보세요" 안내 |
