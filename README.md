...
1. **R-03a/R-03b 동사 분류** — 금지 9개 / 구체화 권장 13개 분류가 현장에 맞는가?
   - 금지(reject): 처리/진행/관리/대응/지원/파악/준비/고도화/리드
   - 권장(warning): 검토/개선/최적화/정리/공유/조율/협의/반영/분析/평가/담당/보조/피드백
   - `확인한다`는 표준 동사로 복원 (원본 가이드라인 기준)
2. **R-07 타동사 목록** — 조회/입력/수정/저장/추출/비교/집계/기록/첨부/판정/승인/반려/전송/배포/제공/체크 (16개), R-07 위반 시 **reject**
...
> 본 섹션은 팀원들과의 토의를 위해 최초 가이드라인(`First_Guideline.md`)과 현재 구현의 차이를 정리합니다.

#### 원본 v1 (First_Guideline.md) — 팀 초안

- **체크리스트 구조**: 3개 섹션 (구조·형식 / 판단·의사결정 / 산출물 및 예외처리) + 20개 Agent 규칙(R-01~R-20)
- **동사 목록**: "L7에서 피해야 할 용어" 단일 목록 (25개+ 금지), 표준 동사 사전 (약 20개)
- **수행 위치**: 라벨에 시스템/화면/문서명 포함 필수 (필수 항목)
- **판단 규칙**: R-12(판단 동사 존재) → R-13(기준 키워드) → R-14(결과값) 3단계 체인
- **품질 활용**: R-18~R-20 (자동화 가능성 / 사람 판단 / 재작업 가능성 점수)

#### 현재 v2 구현 — 주요 변경 내역

| 항목 | v1 원본 | v2 현재 구현 | 변경 근거 |
|:-----|:--------|:------------|:---------|
| **구조 계층** | 없음 | S-01~S-11 (Tier 1) 신설 | bpmnlint / 7PMG 학술 기반 — 플로우 구조 오류 선제 탐지 |
| **동사 정책 분류** | "피해야 할 용어" 단일 목록 | 금지(reject) / 구체화 권장(warning) 이분화 | 현장 혼란 줄이기 — "이 동사는 절대 안 된다"와 "이 동사는 더 나은 게 있다"를 구분 |
| **금지 동사 수** | 25개+ (파악한다, 보조한다, 담당한다, 검수한다 등 포함) | 9개 reject (처리/진행/관리/대응/지원/파악/준비/고도화/리드) | 과도한 reject → 현장 저항 완화; 나머지는 warning으로 유지 |
| **확인한다** | 표준 동사 목록에 포함 | v2 초기 warning 오분류 → **v2.1에서 표준 동사로 복원** | 원본 의도 복원; "확인" 자체는 구체적 행위 가능 |
| **공유한다** | 금지 동사 | 구체화 권장(warning) | 단순 사용 금지보다 "안내한다/발송한다" 대안 제시가 실무에 유용 |
| **R-07 목적어 누락** | 필수(Reject) | v2 초기 warning → **v2.1에서 reject 복원** | 원본 "목적어 명확성 필수" 항목과 일치 |
| **수행 위치 (시스템명)** | 라벨에 포함 필수 | R-04: 감지 후 메타데이터 필드로 분리 안내 | 라벨과 메타데이터 분리 → 구조적 일관성; UI에서 systemName 필드 별도 관리 |
| **판단 규칙** | R-12→R-13→R-14 3단계 체인 | R-08 단일 규칙 (기준 힌트 키워드 탐지) | 현장 사용 간소화; 3단계 연쇄는 false positive 발생 多 |
| **표준 동사 추가** | 전송하다, 배포하다, 제공한다, 체크한다 포함 | 동일 목록으로 TRANSITIVE_VERBS에 추가 | 원본 복원 (누락됐던 항목) |
| **R-18~R-20 품질 활용** | 점수 산정 규칙 | Tier 3 (AI 시그널 A-01~A-03) 별도 Phase | BEF4LLM 원칙 — 규칙 기반 점수와 AI 판단 분리 |

#### 토의 포인트

- **공유한다 처리**: 원본은 금지였으나 v2는 warning으로 완화. 현장에서 "공유"라는 표현이 얼마나 자주 쓰이는지에 따라 재조정 가능
- **보조한다/담당한다**: 원본에서 금지였으나 v2에서 warning으로 조정. "지원한다"(reject)와의 일관성 재검토 필요
- **수행 위치**: 원본 필수 항목이었으나 v2에서 메타데이터로 분리. 라벨 간결성 vs. 원본 의도 중 무엇을 우선할지 팀 결정 필요
- **R-06 주어 조건**: 원본에 없던 신규 규칙 (스윔레인 미사용 시 주어 안내). 현장 적용 시 반응 확인 필요
