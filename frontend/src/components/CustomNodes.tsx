...
export const DecisionNode = memo(({ id, data, selected }: NodeProps<FlowNodeData>) => {
  ...
  const diamondFilter = selected
    ? 'drop-shadow(0 0 12px rgba(245,158,11,0.5))'
    : sc ? `drop-shadow(0 0 5px ${sc})` : undefined;
  return (<div className="relative" style={{ width: 160, height: 160 }}>
    ...
    <div className="absolute inset-0 flex items-center justify-center" style={{ clipPath: 'polygon(50% 0%,100% 50%,50% 100%,0% 50%)', background: bg, filter: diamondFilter }} onDoubleClick={ie.startEdit}>
      {ie.editing ? (
    ...
    {sc && badge && <div className="absolute w-5 h-5 rounded-full flex items-center justify-center text-[10px] z-20" style={{ top: 30, right: 44, background: `${sc}22`, border: `1.5px solid ${sc}`, color: sc }}>{badge}</div>}
  </div>);
});
