...
export default function L7GuideModal({ onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="relative w-[680px] max-h-[80vh] overflow-hidden rounded-2xl flex flex-col"
        style={{ background: '#0f1729', border: '1px solid rgba(148,163,184,0.15)', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-5 pb-3 flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-100">L7 라벨 작성 가이드라인</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4 text-xs text-slate-300">
          {tab === '라벨 규칙' && (
            <>
              <Section title="기본 형식">
                <Row id="R-01" sev="warning" label="길이 부족" desc="4자 미만이면 의미 전달이 어렵습니다." />
                <Row id="R-02" sev="warning" label="길이 초과" desc="100자 초과 시 핵심 동작이 흐려집니다." />
              </Section>
              <Section title="동사 규칙">
                ...
                <div className="rounded-lg p-3 space-y-1" style={{ background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.18)' }}>
                  <div className="text-[10px] font-semibold text-amber-400 uppercase tracking-wider mb-1.5">R-03b · 구체화 권장 (대안 제시)</div>
          ...
          {tab === '구조 규칙' && (
            <>
              ...
              <Section title="필수 구조">
                <Row id="S-01" sev="warning" label="종료 노드 필수" desc="프로세스 범위를 명확히 하려면 종료 노드가 최소 1개 있어야 합니다." />
                <Row id="S-02" sev="warning" label="빈 라벨 방치" desc="'새 태스크' 같은 기본값은 반드시 구체적인 라벨로 바꿔주세요." />
                <Row id="S-03" sev="warning" label="고아 노드" desc="어느 단계와도 연결되지 않은 노드는 플로우에서 제외된 것과 같습니다." />
              </Section>
              <Section title="분기/연결 규칙">
                <Row id="S-05" sev="warning" label="암묵적 분기" desc="프로세스 노드에서 2개 이상 분기할 경우 판단 노드로 조건을 명시하세요." />
                <Row id="S-06" sev="warning" label="중복 연결" desc="동일한 출발→도착 연결이 2개 이상이면 하나를 삭제하세요." />
                <Row id="S-07" sev="warning" label="무의미 판단" desc="판단 노드에서 나가는 경로가 1개뿐이면 일반 프로세스 노드로 변경하세요." />
                <Row id="S-08" sev="warning" label="조건 라벨 누락" desc="판단 노드의 모든 분기에 '예/아니오' 또는 구체적 조건을 적어주세요." />
              </Section>
              <Section title="복잡도 관리">
                <Row id="S-09" sev="warning" label="다중 시작" desc="시작 노드가 2개 이상이면 의도적 구조인지 확인하세요." />
                <Row id="S-10" sev="warning" label="과다 분기" desc="판단 노드에서 4개 이상 분기 시 2단계 판단으로 나누면 가독성이 좋아집니다. (7PMG G2)" />
                <Row id="S-11" sev="warning" label="모델 복잡도" desc="노드가 50개를 초과하면 서브프로세스로 분해하는 것을 권장합니다. (7PMG G7)" />
              </Section>
            </>
          )}
...
function Row({ id, sev, label, desc }: { id: string; sev: 'reject' | 'warning' | 'suggest'; label: string; desc: string }) {
  const colors = {
    reject:  { bg: 'rgba(239,68,68,0.08)',   border: 'rgba(239,68,68,0.2)',   text: '#f87171' },
    warning: { bg: 'rgba(251,191,36,0.07)',  border: 'rgba(251,191,36,0.2)',  text: '#fbbf24' },
    suggest: { bg: 'rgba(148,163,184,0.06)', border: 'rgba(148,163,184,0.15)', text: '#94a3b8' },
  }[sev];
  return (
    <div className="flex items-start gap-2.5 rounded-lg px-3 py-2" style={{ background: colors.bg, border: `1px solid ${colors.border}` }}>
      <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[9px] font-semibold mt-0.5" style={{ color: colors.text, border: `1px solid ${colors.border}` }}>{id}</span>
      <div>
